...
class TargetContext extends Context {
	...
	async _check(..._prerequisites) {
		...
		for (let p of prerequisites) {
			...
			if (p.__lateCheckDependency) {
				tasks.rear.push(this.definitions.run(p, this));
			} else {
				tasks.front.push(this.definitions.run(p, this));
			}
		}
		await Promise.all(tasks.front);
		await Promise.all(tasks.rear);
